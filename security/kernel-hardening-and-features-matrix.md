---
layout: default
---

## Kernel Hardening and Security Feature Matrix

- [Ubuntu Kernel Hardening](https://wiki.ubuntu.com/SecurityTeam/Roadmap/KernelHardening)
- [Fedora Security Features Matrix](https://fedoraproject.org/wiki/Security_Features_Matrix)
- [Ubuntu Security Features Matrix](https://wiki.ubuntu.com/Security/Features)
- [Debian Hardening](https://wiki.debian.org/Hardening)
- [SUSE Security Features in the Kernel](https://www.suse.com/documentation/sles-12/book_hardening/data/sec_sec_prot_general_kernel.html)
- [openSUSE-gardened](https://github.com/kdave/openSUSE-gardened)
- [openSUSE Kernel Site](http://kernel.opensuse.org/)
- [Hardening CentOS](https://wiki.centos.org/HowTos/OS_Protection)


[Back](../)

